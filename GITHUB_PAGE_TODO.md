...
### 2.1 Safe Accessors (안전한 데이터 추출 - `std::optional`)
데이터 타입 불일치나 키가 없을 경우 발생하는 에러 처리를 심플하고 우아하게 만들기 위해 C++17 `std::optional`을 적극 도입합니다. 예외(Throw) 남용을 방지합니다.
- [ ] `std::optional<int64_t> get_int64() const` : NumberRaw 파싱 후 반환, 실패 시 `nullopt`
- [ ] `std::optional<double> get_double() const` : 부동소수점 파싱
- [ ] `std::optional<bool> get_bool() const`
- [ ] `std::optional<std::string_view> get_string() const`

### 2.2 Extreme Usability API (세계 최고 수준 사용자 편의성)
- [ ] **Fluent Chaining (Monadic Error Handling)**: `doc["users"][0]["name"]` 접근 시 중간 키가 없어도 Throw 없이 `Null` 객체를 반환하여 무예외 연속 탐색 지원.
- [ ] **Default Value Getters**: `get_string("Unknown")` 처럼 키가 없을 때 지정된 기본값을 반환하는 API 제공.
